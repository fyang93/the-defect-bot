...
The bot is organized as a small layered system: bot runtime, platform adapters, AI, repository CLI, domain operations, and records, with the repository CLI used as the preferred deterministic execution boundary. Recent deepening work has pulled schedule lifecycle concerns behind `ScheduleEngine`, startup/runtime orchestration behind a bot lifecycle module, and OpenCode session lifecycle behind an internal `SessionBroker` seam.
...
- `cli-rules`: durable future-facing assistant rules
- `memory`: repository-local durable notes and preferences
- `custom-toolbox`: narrow project-specific utility workflows
