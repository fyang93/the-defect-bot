...
function diffConfigKeys(before: AppConfig, after: AppConfig): string[] {
  ...
  if (before.telegram.waitingMessages.join("\0") !== after.telegram.waitingMessages.join("\0")) changed.push("telegram.waiting_messages");
  if (before.telegram.waitingMessageRotationSeconds !== after.telegram.waitingMessageRotationSeconds) changed.push("telegram.waiting_message_rotation_seconds");
